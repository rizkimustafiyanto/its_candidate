...
class overtimedetailreport_model extends CI_Model
{
    function GetOvertimeReport($overtimereport_parameter)
    {
        $procedure = "call usp_xt_overtime_detail_report_select(?,?,?,?,?)";
    ...
    {
        $procedure = "call usp_xt_overtime_detail_report_cabang_select(?,?,?,?,?,?)";
        $sql_query = $this->db->query($procedure, $overtimereport_parameter);
        mysqli_next_result($this->db->conn_id);
        if ($sql_query->num_rows() > 0) {
            return $sql_query->result();
