...
class sppd_report_controller extends BaseController
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('reportdetail/sppd_report_model');
    ...
    {
        $this->global['pageTitle'] = 'CodeInsect : SPPD Report';
        $this->loadViews("reportdetail/sppdreport", $this->global, NULL, NULL);
    }

    function GetSPPDReport()
    {
        ...
        if ($this->session->userdata('role_id') == '1' || $this->session->userdata('role_id') == '5') {
            ...
            $sppd_report_parameter = array(
                'p_employee_id' => 0,
                'p_company_id' => $companypusat,
                'p_company_brand_id' => $company_brand_id_pusat,
                'p_date_1' => $date_1, 'p_date_2' => $date_2, 'p_flag' => 0
            );
            $data['SPPDReportRecords'] = $this->sppd_report_model->GetSPPDReport2($sppd_report_parameter);
        }
        ...
        $this->loadViews("reportdetail/sppdreport", $this->global, $data, NULL);
    }

    function GetDefaultSPPDReport()
    {
        ...
        if ($this->session->userdata('role_id') == '1' || $this->session->userdata('role_id') == '5') {
            ...
            $sppd_report_parameter = array('p_sppd_id' => 0, 'p_employee_id' => 0, 'p_date_1' => 0, 'p_date_2' => 0, 'p_flag' => 0);
            $data['SPPDReportRecords'] = $this->sppd_report_model->GetSPPDReport($sppd_report_parameter);
        }
